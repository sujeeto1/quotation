...
import { 
  ...
  X, CheckSquare, BookmarkPlus, ChevronDown, ChevronRight, ChevronLeft, Settings2, Edit3, Check, Search, Download, Info, Upload, RefreshCw, Link, Package, Copy, Settings, Calendar, AlertCircle, ChevronUp, FileText, MapPin, ArrowUp, ArrowDown, CloudSync, Briefcase, DollarSign, UserCheck, Layers, Send
} from 'lucide-react';
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {library.exclusions.map((exc, i) => {
                        ...
                        return (
                          <button key={i} onClick={() => toggleTabExclusion(exc)} className={`px-4 py-2 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${active ? 'bg-red-600 border-red-600 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-500 hover:bg-white'}`}>
                            {exc}
                          </button>
                        );
                      })}
